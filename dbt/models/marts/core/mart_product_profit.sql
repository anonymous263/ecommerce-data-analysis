...
with items as (
    select
        ...
        date_sk,
        product_sk,
        -- conformed FKs + degenerate status inherited from fact_order_item (which
        -- itself inherits them from the order header) so profit-by-line slices by
        -- country/customer/payment directly and status splits completed vs refunded
        country_sk,
        customer_sk,
        payment_method_sk,
        order_status,
